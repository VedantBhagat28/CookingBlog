...
exports.homepage = async (req, res) => {
  try {
    ...
    const food = { latest, thai, indian, chinese };
...
exports.exploreCategories = async (req, res) => {
  try {
    ...
    res.render("categories", {
      title: "Cooking Blog - categorieis",
      categories,
    });
  } catch (error) {
    res.status(500).send({ message: error.message || "Error Occured" });
...
exports.searchRecipe = async (req, res) => {
  try {
    ...
    res.render("search", {
      title: "cooking BLog - Search vedantbhagt",
      recipe,
    });
  } catch (error) {
    res.status(500).send({ message: error.message || "Error Occured" });
...
//   try {
//     await Recipe.deleteOne({ name: 'Dal Tadka' });
//   } catch (error) {
//     console.log(error);
//   }
// }
...
//  easier way to insert dummy data in mongodb
